...
from tess_enumerator_tests.pytest_constants import ENCODING, TESTABLE_DATA_DIR, TESTABLE_BUCKET_URL_BASE, \
    TESTABLE_TEMPLATE_DIR
from tess_enumerator_tests.pytest_datatypes import FITSTestFile

def load_template(template_name: str) -> str:
    template_path = os.path.join(TESTABLE_TEMPLATE_DIR, template_name)
...
def fits_files():
    ...
    name = 'messier-17'
    url = None
    fits_headers = [fits.open(os.path.join(TESTABLE_DATA_DIR, '502nmos.fits'))]
    cache_url = f'{TESTABLE_BUCKET_URL_BASE}/502nmos.fits'
    ...
    fits_headers = json_file_to_fits_headers(f'{TESTABLE_DATA_DIR}/tess-s0022-4-4-cube-headers.json')
    cache_url = None
    records.append(FITSTestFile(name, url, fits_headers, cache_url))
