...
from tess_enumerator_tests.pytest_constants import ENCODING
from tess_enumerator_tests.pytest_utils import aws_credential_filepath, aws_credential_filepath__empty

def test__aws_auth():
    import requests

    from tess_enumerator_tests.pytest_constants import TESTABLE_BUCKET_URL_WITH_MUTABLE_FILE
    from tess_enumerator.auth.aws import AWSAuth

    test_file_filepath = '/tmp/test-file.txt'
    url = TESTABLE_BUCKET_URL_WITH_MUTABLE_FILE
...
def test__load_aws_context__credential_file(aws_credential_filepath, monkeypatch):
    from tess_enumerator.auth import aws as auth_aws
    from tess_enumerator.auth.aws import load_aws_auth_context, AWSService
...
def test__get_canonical_headers():
    from tess_enumerator.auth import aws as auth_aws
    from tess_enumerator.auth.aws import get_canonical_headers, AMZDATE_FORMAT

    from datetime import datetime

    timestamp = datetime.utcnow()
    host = 's3.us-east-1.amazonaws.com'
    expected_ordered_headers = 'host;x-amz-date;x-amz-expires'
    expected_ordered = '\n'.join([
        f'host:{host}',
    ...
    ordered_headers, ordered = get_canonical_headers(timestamp, host, False, 30)
...
def test__get_canonical_headers__payer_request():
    ...
    expected_ordered = f'{expected_ordered}\n'
    ordered_headers, ordered = get_canonical_headers(timestamp, host, True, 30)
    assert ordered_headers == expected_ordered_headers
    assert ordered == expected_ordered
...
def test__get_canonical_querystring__empty():
    from tess_enumerator.auth import aws as auth_aws
    from tess_enumerator.auth.aws import get_canonical_querystring

    url = 'https://s3.{TEST_REGION}.amazonaws.com/{TEST_BUCKET_NAME}/test-file.txt'
...
def test__get_payload_hash():
    ...
    from tess_enumerator.auth import aws as auth_aws
    from tess_enumerator.auth.aws import get_payload_hash
...
def test__sign__str_input():
    ...
    from tess_enumerator.auth import aws as auth_aws
    from tess_enumerator.auth.aws import sign
...
def test__get_signature_key(aws_credential_filepath):
    from tess_enumerator.auth.aws import get_signature_key, DATESTAMP_FORMAT, sign, load_aws_auth_context
...
# 
#     TESS_BUCKET_NAME = 'tess-enumerator'
#     TESS_CUBE_PREFIX = 'list-objects/'
# 
#     from tess_enumerator.auth.aws import AWSAuth
# 
#     params = {
...
#         'max-keys': 20,
#         'Prefix': TESS_CUBE_PREFIX,
#         # 'start-after': 'StartAfter'
#     }
#     host = f'https://{TESS_BUCKET_NAME}.s3.amazonaws.com'
#     url = urlencode(params)
#     url = '/?{url}'
#     url = f'{host}{url}'
#     resp = requests.get(url, auth=AWSAuth(request_payer=True), headers={
