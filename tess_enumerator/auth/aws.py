...
from tess_enumerator.constants import ENCODING
...
def get_canonical_headers(timestamp: datetime, host: str, request_payer: bool, x_amz_expires: str) -> str:
    '''
...
class AWSAuth(AuthBase):
    ...
    def __call__(self: PWN, request: 'requests.Request') -> 'requests.Request':
        ...
        x_amz_date = timestamp.strftime('%Y%m%dT%H%M%SZ')
        x_amz_expires = '30'

        payload_hash = get_payload_hash(request.body)
        signed_headers, canonical_headers = get_canonical_headers(
                timestamp,
                request_host,
                self._request_payer, x_amz_expires)
        canonical_request = '\n'.join([
            request.method,
        ...
        request.headers['x-amz-date'] = x_amz_date
        request.headers['x-amz-content-sha256'] = payload_hash
        request.headers['x-amz-expires'] = x_amz_expires
